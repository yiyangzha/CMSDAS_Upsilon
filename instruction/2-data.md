...
In this section, we will use a program called `ntuplizer` to process these datasets in order to:
- Select events containing two muons;
- Convert the file format to an `ntuple`, which is more convenient for direct use.
...
The output of `mm_tree->Print()` lists all the available variables in our sample file. The meanings of the variables that we will use are as follows:
...
| `vProb` | `float` | vertex-fit probability of the dimuon candidate; larger values indicate a better fit quality and thus are more likely to represent real $\Upsilon$ rather than background |
...
**Note**: there is no need to wait for the jobs to finish completely. Due to time constraints, the subsequent steps will use the data samples that have already been prepared.
...
> #### **Task**
> The current program draws variable distributions without applying any selections. To obtain variable distributions for events that match the analysis selection, the following requirements should be added inside the event loop:
> - acceptance kinematics for muons: $|\eta|<2.0$ and $p_\mathrm{T}>3.1$ GeV,
> - trigger (`HLT_Dimuon0_Upsilon`) passed,
> - opposite-charge dimuon requirement (`charge == 0`),
> - single-candidate requirement (`nonia == 1`),
> - $vProb>0.01$.
>
> **Hints**:
...
> }
> ```
> 2. Besides adding the selections, remember to also edit the save path of the figures.
